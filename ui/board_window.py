...
class BoardApp(tk.Tk):
    ...
    def _cfg_to_board(self, d: dict) -> BoardConfig:
        from logic.board_map import PlayArea, StorageLayout
    ...
    def _calibrate_handler(self, square: str):
        ...
        # Update editor view's board configuration
        if hasattr(self, 'editor_view') and self.editor_view:
            self.editor_view.board_cfg = self.board_cfg
    ...
    def _switch_profile(self):
        ...
        # Clear board state when switching profiles to avoid invalid square references
        self.board_state.clear_virtual()
        self.board_state.clear_physical()
    ...
    def _poll_servos(self):
        """Poll servo status (status displays removed in unified interface)."""
        # Note: Servo status displays were removed when merging to unified editor view
        # The servo controller still tracks state internally
        # This method now just maintains the polling cycle for potential future use
